...
public class EnemyBehaviour : MonoBehaviour {
	...
	public bool PlayerInSight(GameObject player, GameObject npc)
	{
		...
		if (angle < fieldOfViewAngle) {
			RaycastHit hit;
			if (Physics.Raycast(npc.transform.position + new Vector3(0, 0.925f, 0), toPlayer.normalized, out hit, sightRange)) {
...
public class PatrolState : FSMState
{
	...
	{
		//Check line of sight.
		if (npc.GetComponent<EnemyBehaviour>().PlayerInSight(player, npc) == true) {
	...
	public override void Act(GameObject player, GameObject npc)
	{
		...
		Vector3 distance = npc.transform.position - waypoints[currentWayPoint].position;
...
public class ChasePlayerState : FSMState
{
	...
	{
		//Check line of sight.
		if (npc.GetComponent<EnemyBehaviour>().PlayerInSight(player, npc) == false) {
	...
	public override void Act(GameObject player, GameObject npc)
	{
		...
		if (distance.magnitude > stopDist) {
